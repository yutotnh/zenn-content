...
- [editor.wordSegmenterLocales](#editor.wordsegmenterlocales)の設定を修正
    - `"ja"` → `["ja"]`
...
- [files.autoGuessEncoding](#files.autoguessencoding)の説明を修正
    - [files.candidateGuessEncodings](#files.candidateguessencodings)への言及を追加
...
推測機能は完璧ではなため別のエンコーディングとして判定されることがありますが、後述の[files.candidateGuessEncodings](#files.candidateGuessEncodings)で推測する際の候補を設定することで、誤検知を減らすことができます。

複数のエンコーディングを使わない場合は、本機能は無効にして[files.encoding](vscode://settings/files.encoding)でエンコーディングを指定することをオススメします。
